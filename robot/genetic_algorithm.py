...
######################################################################
    ...
    def select_parents(self, parents):
        ...
        for i in range(0, len(parents), 2):
            pair_parents.append((parents[i], parents[i+1]))
        return pair_parents
    ...
    def mutate_offspring(self, p):
        ...
        population_prob = 1/self.population_size
        chromosome_prob = 1/self.chromosome_length
        mutation_rate = random.uniform(population_prob, chromosome_prob)
    
        s = ''
        #print("before:", p.bin)
        for i in p:
            r_rand = random.randrange(2)
            # print(mutation_rate, r_rand)
            if mutation_rate > r_rand:
                #print("HAHAH")
                if i == 1:
                    s += '0'
                else:
    ...
    def evaluate_candidates(self, candidates): 
        ...
        f_avg = (sum_of_pairs / len(candidates)*28)/self.population_size
        final_individuals = []
        if f_avg > 0:
            relative_fitness = []
            ...
            for _,fitness_ind in relative_fitness:
                ...
                prob.append(p)
            while len(final_individuals) < len(candidates):
                for i in range(len(candidates)):
                    if len(final_individuals) < len(candidates):
                        num = random.randrange(2)
                        if num > prob[i]:
                            final_individuals.append(candidates[i])
        else: 
            while len(final_individuals) < len(candidates):
...
#     #print(len(fit))
#     child = xyz.select_parents(cands)
#     #print(child)
#     mutate = xyz.mutate_offspring(cands[4])
# main()
